...
\begin{abstract}
\small{
This paper aims to determine which words (if any) in wine reviews best
predict their corresponding rating by utilising a Random Forest model
and an OLS regression analysis. The Random Forest model is used as a
variable selector on tokenized wine review words. The author's results
are statistically significant at the 1\% level and indicate that a
