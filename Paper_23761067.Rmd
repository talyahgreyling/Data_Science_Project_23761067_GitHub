...
To uncover the underlying relationship regarding which words used in wine reviews are the best predictors of high ratings a two-step statistical modelling process is employed. Step 1 entails utilising a random forest model as a method to select the 20 words (or 'variables') that best predict a high rating. These variables are then fed into a simple OLS regression in 2 step of the modelling process to calculate coefficients and standard errors and inspect the significance of these words in predicting high ratings. 

To employ step 1 the tokenised data was first converted into a document term matrix of words each relinked to their respective ratings. 
