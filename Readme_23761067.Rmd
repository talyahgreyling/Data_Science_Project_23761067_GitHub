...
fig1 <- ggplot(words_ratings_counts, aes(x = ave_rating)) +
  ...
  labs(title = "Frequency of average ratings of review tokens",
       x = "Average Rating", 
       y = "Frequency")
  theme = my_theme
fig1

#ggsave("Figures/fig1.png", plot = fig1, width = 6, height = 5, dpi = 300)
...
# Create Word Cloud
fig2 <- wordcloud2(words_popular, size = 1.6, color = palette, backgroundColor = "white")
fig2
...
* Visualise top 20 with a wordcloud & variable importance plot 
...
ggplot() + 
  geom_density(data = training_ratings, aes(rating.x), 
               alpha = 0.5, color = "#CE677A") + 
  geom_density(data = testing_ratings, aes(rating.x), 
...
# Set regular grid
grid_vals <- grid_regular(mtry(range = c(4, 12)), levels = 5)
...
#### Figure 2: Word cloud of top 20 words in reviews identified by RandomForest model
```{r fig2 RFwordcloud}
...
# Draw wordcloud
wordcloud2(df_wordcloud, size = 1.2, color = palette, backgroundColor = "white")
...
fruitlist <- read_table("data/fruitlist.txt") #Reads a text file containing fruit-related terms (one per line)
fruitwords <- fruitlist$Fruits %>% 
    unique %>% 
    as.list()
fruit_pattern <- paste(fruitwords, collapse="|")

# Count fruit references 
df_fruit <- data %>%
    group_by(vintage_wine_name) %>%
    summarise(
        Ref_froot = sum(grepl(fruit_pattern, note)) / n(),
        Fruit = toString(fruitwords[unique(grep(fruit_pattern, note))])
    )
